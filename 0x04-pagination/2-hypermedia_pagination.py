...
class Server:
    ...
    def get_hyper(self, page: int = 1, page_size: int = 10) -> Dict:
        ...
        total_pag: int = len(dataset) if dataset else 0
        total_pag = ceil(total_pag / page_size)
        prev_pag: int = (page - 1) if (page - 1) >= 1 else None
        next_pag: int = (page + 1) if (page + 1) <= total_pag else None
